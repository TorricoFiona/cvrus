<?php
/**
 * Agrega columnas para verificación de email si no existen.
...
class EmailVerificationMigrator extends DBAbstract {
    public function migrate(){
        $cols = $this->consultar("SHOW COLUMNS FROM usuario");
        $names = array_map(fn($c) => $c['Field'], $cols);
        $alterStatements = [];
        if(!in_array('email_verificado', $names)){
            $alterStatements[] = 'ADD COLUMN email_verificado TINYINT(1) NOT NULL DEFAULT 0 AFTER password';
        }
        ...
        $sql = 'ALTER TABLE usuario ' . implode(', ', $alterStatements);
